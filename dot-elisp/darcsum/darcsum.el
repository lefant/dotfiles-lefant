...
(defun darcsum-really-record ()
  ...
  (let ((buf (generate-new-buffer " *darcs record*"))
    ...
    ;; trailing space in the diffs.)  Fixme: abstract this into a
    ;; function to start the darcs process.
    ...
    (with-current-buffer buf
      (set (make-local-variable 'darcsum-logfile) tempfile)
...
(defun darcsum-send (recipient)
  ...
  (let ((parent-buf (current-buffer))
    ...
    (with-current-buffer buf
      (set (make-local-variable 'darcsum-changeset-to-record) changeset)
      (set (make-local-variable 'darcsum-parent-buffer) parent-buf)
      (set (make-local-variable 'darcsum-process-arg) recipient))))
...
(defun darcsum-whatsnew (directory &optional arg no-display show-context)
  (interactive
   (let ((root (darcsum-repository-root)))
  ...
  (with-temp-buffer
    ...
    (let* ((process-environment (cons "DARCS_DONT_ESCAPE_TRAILING_SPACES=1"
	   ...
	   (args (append
		  ...
		  (unless (null arg) (list "--look-for-adds"))
